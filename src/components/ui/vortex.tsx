...
export const Vortex = (props: VortexProps) => {
  ...
  const particleCount = props.particleCount ?? 520;
  ...
  const baseHue = props.baseHue ?? 42; // warm / gold
  const rangeHue = 24;
  ...
  const resizeToContainer = (canvas: HTMLCanvasElement, ctx?: CanvasRenderingContext2D) => {
  ...
  const drawParticle = (
    ...
    ctx.strokeStyle = `hsla(${hue},100%,65%,${fadeInOut(life, ttl) * 0.9})`;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
  ...
  const updateParticle = (i: number, ctx: CanvasRenderingContext2D, w: number, h: number) => {
    const i2 = 1 + i,
      i3 = 2 + i,
      i4 = 3 + i,
      i5 = 4 + i,
